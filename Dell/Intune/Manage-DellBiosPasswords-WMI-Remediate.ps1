<#
    .DESCRIPTION
        ...
        non-Dell hardware reporting (SKIPPED: pw out-of-scope).
...
param(
    ...
    [System.IO.FileInfo]$LogFile = "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs\Manage-DellBiosPasswords-WMI-Remediate.log",
...
function Get-DellAdminPasswordSet
{
    #Query the Dell PasswordObject WMI class to determine if the Admin password is currently set.

    try
    {
        $PasswordObject = Get-CimInstance -Namespace 'root\dcim\sysman\wmisecurity' -ClassName 'PasswordObject' -ErrorAction Stop
    }
    catch
    {
        Write-LogEntry -Value "Dell PasswordObject WMI query failed: $($_.Exception.Message)" -Severity 2
        return $null
    }
...
function Invoke-DellSetAdminPassword
{
    #Call Dell SetNewPassword for the Admin password (the old/new password shape selects set vs change vs clear). Returns @{ Status; Threw; ExceptionMessage }.
    ...
    if ($WhatIfPreference)
    {
        ...
        Write-LogEntry -Value "WhatIf: would call Dell SetNewPassword [Admin] - $ShapeLabel" -Severity 1
    ...
    {
        return @{ Status = -1; Threw = $true; ExceptionMessage = "Get-CimInstance SecurityInterface failed: $($_.Exception.Message)" }
    }
    ...
    {
        $OldBytes = $Encoder.GetBytes($OldPassword)
        $CimArgs = @{
            SecType     = [uint32]1
            SecHndCount = [uint32]$OldBytes.Length
            SecHandle   = [byte[]]$OldBytes
            NameId      = 'Admin'
    ...
    try
    {
        $Result = Invoke-CimMethod -InputObject $SecurityInterface -MethodName 'SetNewPassword' -Arguments $CimArgs -ErrorAction Stop
    }
    catch
    {
        return @{ Status = -1; Threw = $true; ExceptionMessage = "Invoke-CimMethod SetNewPassword threw: $($_.Exception.Message)" }
    }
...
function Invoke-RemediateSetOrReapply
{
    ...
    try
    {
        ...
        $Op = Invoke-DellSetAdminPassword -OldPassword '' -NewPassword $NewPw
        if ($Op.Threw)
        {
            Write-LogEntry -Value "Dell WMI threw on $Action`: $($Op.ExceptionMessage)" -Severity 3
            return @{ Stdout = "FAILED: pw $($Action.ToLower()) v=$TargetVersion (WMI threw)"; ExitCode = 1 }
        }
        if ($Op.Status -ne 0)
        {
            Write-LogEntry -Value "Dell SetNewPassword returned status=$($Op.Status) on $Action" -Severity 3
            return @{ Stdout = "FAILED: pw bios-set-failed status=$($Op.Status) (action=$($Action.ToLower()) v=$TargetVersion)"; ExitCode = 1 }
        }
        if (-not $WhatIfPreference)
        {
            $PostSet = Get-DellAdminPasswordSet
            if ($PostSet -ne $true)
            {
                Write-LogEntry -Value "$Action`: SetNewPassword reported success but IsPasswordSet=$PostSet" -Severity 3
...
{
    #Rotate to the target password: walk the fallback list to unlock, set the new password, verify by readback, then advance the marker.
    ...
    try
    {
        ...
        foreach ($CandidateVer in $Plan.FallbackOrder)
        {
            ...
            $Op = Invoke-DellSetAdminPassword -OldPassword $OldPw -NewPassword $NewPw
            if ($Op.Threw)
            {
                Write-LogEntry -Value "Dell WMI threw during rotate: $($Op.ExceptionMessage)" -Severity 3
            ...
            if ($Op.Status -eq 0)
            {
                ...
                Write-LogEntry -Value "Unlock succeeded with v=$CandidateVer (Dell status=0)" -Severity 1
                break
            }
        ...
        if (-not $WhatIfPreference)
        {
            ...
            if ($PostSet -ne $true)
            {
                Write-LogEntry -Value "Rotate: SetNewPassword reported success but IsPasswordSet=$PostSet" -Severity 3
                return @{ Stdout = "FAILED: pw set-readback-mismatch v=$TargetVersion"; ExitCode = 1 }
            }
        }
        Set-PasswordMarker -BasePath $MarkerBasePath -Version $TargetVersion -CertThumbprint $Payload.CertThumbprint
...
function Invoke-RemediateClear
{
    ...
    try
    {
        foreach ($CandidateVer in $Plan.FallbackOrder)
        {
            ...
            $Op = Invoke-DellSetAdminPassword -OldPassword $OldPw -NewPassword ''
            if ($Op.Threw)
            {
                Write-LogEntry -Value "Dell WMI threw during clear: $($Op.ExceptionMessage)" -Severity 3
            ...
            if ($Op.Status -eq 0)
            {
                $Unlocked = $true
                $UsedVersion = $CandidateVer
                Write-LogEntry -Value "Clear succeeded with unlock v=$CandidateVer (Dell status=0)" -Severity 1
                break
            }
        ...
        if (-not $WhatIfPreference)
        {
            ...
            if ($PostSet -ne $false)
            {
                Write-LogEntry -Value "Clear: SetNewPassword reported success but IsPasswordSet=$PostSet" -Severity 3
                return @{ Stdout = "FAILED: pw clear-readback-mismatch (still IsPasswordSet=true)"; ExitCode = 1 }
            }
        }
...
function Invoke-RemediateDrifted
{
    ...
    try
    {
        ...
        foreach ($CandidateVer in $Plan.FallbackOrder)
        {
            ...
            if ($Op.Threw)
            {
                Write-LogEntry -Value "Dell WMI threw during drifted-rotate: $($Op.ExceptionMessage)" -Severity 3
            ...
            if ($Op.Status -eq 0)
            {
                $Unlocked = $true
                $UsedVersion = $CandidateVer
                Write-LogEntry -Value "Drifted-unlock succeeded with v=$CandidateVer (Dell status=0)" -Severity 1
                break
            }
        ...
        if (-not $WhatIfPreference)
        {
            ...
            if ($PostSet -ne $true)
            {
                Write-LogEntry -Value "Drifted: SetNewPassword reported success but IsPasswordSet=$PostSet" -Severity 3
                return @{ Stdout = "FAILED: pw set-readback-mismatch v=$TargetVersion"; ExitCode = 1 }
            }
        }
        Set-PasswordMarker -BasePath $MarkerBasePath -Version $TargetVersion -CertThumbprint $Payload.CertThumbprint
...
Write-LogEntry -Value "START - Dell BIOS password remediation (Intune) v$Version" -Severity 1
...
if (-not $SkipManufacturerCheck)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($Manufacturer) -and $Manufacturer -notlike 'Dell*')
    {
        ...
        Write-LogEntry -Value "END - Dell BIOS password remediation (out of scope)" -Severity 1
        Write-Output "SKIPPED: pw out-of-scope (not Dell)"
        exit 0
    }
...
#Current state check.
$BiosSet = Get-DellAdminPasswordSet
Write-LogEntry -Value "BIOS IsPasswordSet=$BiosSet" -Severity 1
...
Write-LogEntry -Value "END - Dell BIOS password remediation (exit $ExitCode)" -Severity 1
