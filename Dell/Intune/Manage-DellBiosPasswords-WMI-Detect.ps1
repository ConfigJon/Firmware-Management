<#
    .DESCRIPTION
        ...
        or 1 (non-compliant). Skips on non-Dell hardware reporting (COMPLIANT: pw out-of-scope).
...
param(
    ...
    [System.IO.FileInfo]$LogFile = "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs\Manage-DellBiosPasswords-WMI-Detect.log",

    #Internal: bypasses the manufacturer guard so the script can be tested on non-Dell hardware.
    [Parameter(DontShow)]
...
Write-LogEntry -Value "START - Dell BIOS password detection (Intune) v$Version" -Severity 1
...
if (-not $SkipManufacturerCheck)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($Manufacturer) -and $Manufacturer -notlike 'Dell*')
    {
        ...
        Write-LogEntry -Value "END - Dell BIOS password detection (out of scope)" -Severity 1
        Write-Output "COMPLIANT: pw out-of-scope (not Dell)"
        exit 0
    }
...
try
{
    ...
    if ($null -eq $BiosSet)
    {
        Write-LogEntry -Value "BIOS IsPasswordSet query failed - DEGRADED classification" -Severity 2
    }
    else
    {
    ...
    $Result = Get-PasswordClassification -TargetVersion $TargetVersion -Marker $Marker -BiosSet $BiosSet
}
catch
{
...
Write-LogEntry -Value "END - Dell BIOS password detection" -Severity 1
