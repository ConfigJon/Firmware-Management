<#
    .DESCRIPTION
        ...
        or 1 (non-compliant / degraded). Detection only reads the BIOS (Dell settings reads are
        ...
        setting. Skips on non-Dell hardware reporting (COMPLIANT: settings out-of-scope).
...
param(
    ...
    #Opt-in: BIOS has no admin password. Settings are read unauthenticated. Must match the paired -Remediate script.
    [Parameter(Mandatory = $false)]
    [switch]$NoPassword,
    ...
    [System.IO.FileInfo]$LogFile = "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs\Manage-DellBiosSettings-WMI-Detect.log",
...
function Get-SettingsClassification
{
    ...
    if ($null -eq $CurrentBiosValues)
    {
        return @{
            Status = 'DEGRADED'
            Stdout = 'DEGRADED: settings bios-query-failed (Dell BIOS attribute namespace unreachable)'
...
Write-LogEntry -Value "START - Dell BIOS settings detection (Intune) v$Version" -Severity 1
...
if (-not $SkipManufacturerCheck)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($Manufacturer) -and $Manufacturer -notlike 'Dell*')
    {
        ...
        Write-LogEntry -Value "END - Dell BIOS settings detection (out of scope)" -Severity 1
        Write-Output 'COMPLIANT: settings out-of-scope (not Dell)'
        exit 0
    }
...
if ($DesiredSettings.Count -eq 0)
{
    ...
    Write-LogEntry -Value 'END - Dell BIOS settings detection (no work)' -Severity 1
...
try
{
    ...
    if ($null -eq $CurrentBiosValues)
    {
        Write-LogEntry -Value "BIOS attribute query returned null (DEGRADED)" -Severity 2
    }
    else
    {
...
Write-LogEntry -Value "END - Dell BIOS settings detection" -Severity 1
