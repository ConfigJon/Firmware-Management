    ...
    else
    {
        if ($NULL -ne $Module)
        {
            while ($NULL -ne $Module)
            {
                ...
                try
                {
                    $Module | Uninstall-Module -Force | Out-Null
                }
                catch
                {
                    try
...
Function Install-DellBIOSProviderRemote
{
    ...
    try
    {
        Install-Module -Name DellBIOSProvider -Force | Out-Null
    }
    catch
    {
        Write-LogEntry -Value "Unable to install the DellBIOSProvider module from the PowerShell Gallery" -Severity 3
        throw "Unable to install the DellBIOSProvider module from the PowerShell Gallery"
    }
    if (!($Error))
    {
        if ($NULL -ne $Version)
...
{
    $ErrMsg = "The current SMBIOS version is $BiosVerFull. The mininum supported SMBIOS version is 2.3"
    Write-LogEntry -Value $ErrMsg -Severity 3
    throw $ErrMsg
}
...
if ($DllFailure)
{
    $ErrMsg = "One or more .dll files were not found or failed to copy"
    Write-LogEntry -Value $ErrMsg -Severity 3
    throw $ErrMsg
}
...
if ($ModulePath)
{
    ...
    if (Test-Path "$ModulePath\DellBIOSProvider.psd1")
    {
        try
        {
            ...
            $SourceVersion = Get-Content "$ModulePath\DellBIOSProvider.psd1" | Select-String "ModuleVersion ="
...
else
{
    ...
    try
    {
        $WebVersion = Find-Package DellBIOSProvider | Select-Object -ExpandProperty Version
