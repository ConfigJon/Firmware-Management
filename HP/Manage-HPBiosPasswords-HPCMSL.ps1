<#
    ...
    .EXAMPLE
        ...
        Manage-HPBiosPasswords-HPCMSL.ps1 -SetupSet -PowerOnSet -SetupPassword <String1> -PowerOnPassword <String1>

        Set or change an existing setup password and clear a power on password
        Manage-HPBiosPasswords-HPCMSL.ps1 -SetupSet -SetupPassword <String> -OldSetupPassword <String1>,<String2> -PowerOnClear -OldPowerOnPassword <String1>,<String2>

        Clear existing Setup and power on passwords
        Manage-HPBiosPasswords-HPCMSL.ps1 -SetupClear -OldSetupPassword <String1>,<String2> -PowerOnClear -OldPowerOnPassword <String1>,<String2>
    ...
    .CHANGELOG
        ...
        2.0.0 (2026-05-22)
            - Initial release. HPCMSL-based variant of Manage-HPBiosPasswords, derived from the WMI script. Manages the Setup and Power-On passwords using the HP Client Management
              Script Library cmdlets (Set/Clear-HPBIOSSetupPassword, Set/Clear-HPBIOSPowerOnPassword, Get-HPBIOS*PasswordIsSet) instead of direct WMI calls. Runs on both Windows
              PowerShell 5.1 and PowerShell 7. Requires the HPCMSL to be installed (see Install-HPCMSL.ps1).
...
param(
    ...
    [System.IO.FileInfo]$LogFile = "$ENV:ProgramData\ConfigJonScripts\HP\Manage-HPBiosPasswords-HPCMSL.log"
)
...
Function New-HPBiosPassword
{
    ...
    $SetCmd = "Set-HPBIOS$($PasswordType)Password"
    try
    {
        #Attempt to set the power on password when the setup password is already set (the setup password authorizes the change)
        if($SetupPW)
        {
            & $SetCmd -NewPassword $Password -Password $SetupPW -ErrorAction Stop
        }
        #Attempt to set the setup or power on password
        else
        {
            & $SetCmd -NewPassword $Password -ErrorAction Stop
        }
    ...
    catch
    {
        ...
        Write-LogEntry -Value "Exception Message: $($PSItem.Exception.Message)" -Severity 3
    }
}
...
Function Set-HPBiosPassword
{
    ...
    #Check if the password is already set to the correct value (setting the password to itself succeeds only when it is the current password)
    try
    {
        & $SetCmd -NewPassword $Password -Password $Password -ErrorAction Stop
    ...
    catch
    {
        if($OldPassword)
        {
            ...
            While($Counter -lt $OldPassword.Count)
            {
                try
                {
                    & $SetCmd -NewPassword $Password -Password $OldPassword[$Counter] -ErrorAction Stop
...
Function Clear-HPBiosPassword
{
    ...
    $ClearCmd = "Clear-HPBIOS$($PasswordType)Password"
    ...
    if($SetupPW)
    {
        ...
        While($Counter -lt $SetupPW.Count)
        {
            try
            {
                & $ClearCmd -Password $SetupPW[$Counter] -ErrorAction Stop
    ...
    While($Counter -lt $OldPassword.Count)
    {
        try
        {
            & $ClearCmd -Password $OldPassword[$Counter] -ErrorAction Stop
            #Successfully cleared the password
            Set-Variable -Name "$($PasswordType)PWClear" -Value "Success" -Scope Script
            if(Get-TaskSequenceStatus)
            {
...
if($PowerOnPasswordCheck -eq 0)
{
    ...
    if($PowerOnSet)
    {
        ...
        if(($SetupPasswordCheck -eq 1) -or $SetupSet)
        {
