<#
    .DESCRIPTION
        ...
        or 1 (non-compliant). Detection never attempts to unlock the BIOS. Skips on non-HP hardware reporting
        (COMPLIANT: pw out-of-scope). When HP Sure Admin (Enhanced BIOS Authentication Mode) is enabled the
        BIOS requires signed payloads, so the device is reported INCOMPATIBLE (non-compliant).
...
param(
    ...
    [System.IO.FileInfo]$LogFile = "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs\Manage-HPBiosPasswords-WMI-Detect.log",

    #Internal: bypasses the manufacturer guard so the script can be tested on non-HP hardware.
    [Parameter(DontShow)]
...
function Get-PasswordClassification
{
    ...
    param(
        ...
        [Parameter(Mandatory = $false)][AllowNull()][object]$SureAdmin #$true, $false, or $null (query failed)
    )
    $MarkerVersionStr = if ($null -ne $Marker) { "$($Marker.Version)" } else { '-' }

    #Sure Admin coexistence: distinct INCOMPATIBLE state, exit non-compliant so the admin sees the device.
    if ($SureAdmin -eq $true)
    {
        return @{
            Status = 'NONCOMPLIANT'
            Stdout = 'NONCOMPLIANT: pw sure-admin-enrolled (INCOMPATIBLE - signed payloads required)'
            Reason = 'HP Sure Admin (Enhanced BIOS Authentication Mode) is enabled - password lifecycle does not apply'
        }
    }
...
Write-LogEntry -Value "START - HP BIOS password detection (Intune) v$Version" -Severity 1
...
if (-not $SkipManufacturerCheck)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($Manufacturer) -and $Manufacturer -notlike 'HP*' -and $Manufacturer -notlike 'Hewlett*')
    {
        ...
        Write-LogEntry -Value "END - HP BIOS password detection (out of scope)" -Severity 1
        Write-Output "COMPLIANT: pw out-of-scope (not HP)"
        exit 0
    }
...
try
{
    ...
    {
        Write-LogEntry -Value "Sure Admin query failed - proceeding with normal password classification" -Severity 2
    }
    elseif ($SureAdmin)
    {
        Write-LogEntry -Value "Sure Admin (Enhanced BIOS Authentication Mode) is ENABLED - device is INCOMPATIBLE with password lifecycle" -Severity 2
    }
    else
    {
        Write-LogEntry -Value "Sure Admin is not enabled - normal password lifecycle applies" -Severity 1
    }

    $BiosSet = Get-HPSetupPasswordSet
    if ($null -eq $BiosSet)
    {
        Write-LogEntry -Value "BIOS Setup Password IsSet query failed - DEGRADED classification" -Severity 2
    }
    else
    {
    ...
    $Result = Get-PasswordClassification -TargetVersion $TargetVersion -Marker $Marker -BiosSet $BiosSet -SureAdmin $SureAdmin
}
catch
{
...
Write-LogEntry -Value "END - HP BIOS password detection" -Severity 1
