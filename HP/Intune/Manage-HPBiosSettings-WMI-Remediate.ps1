<#
    .DESCRIPTION
        Intune remediation script for HP BIOS settings management. Compares the admin-edited
        $DesiredSettings hashtable to the BIOS-reported state, decrypts the password named by the pw marker
        (CMS material in $IntunePayload), and applies drifted settings via SetBIOSSetting, verifying each by
        readback before writing its per-setting marker. Settings that fail are cached so they do not retry
        forever on hardware that cannot accept them. Skips on non-HP hardware reporting (SKIPPED: settings
        out-of-scope). When HP Sure Admin (Enhanced BIOS Authentication Mode) is enabled this script
        short-circuits (SKIPPED, exit 0) and the -Detect script flags the device INCOMPATIBLE.
...
param(
    ...
    #Opt-in: BIOS has no setup password. Settings are read/applied unauthenticated. Must match the paired -Detect script.
    [Parameter(Mandatory = $false)]
    [switch]$NoPassword,
    ...
    [System.IO.FileInfo]$LogFile = "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs\Manage-HPBiosSettings-WMI-Remediate.log",
...
function Get-SettingsRemediationPlan
{
    ...
    if ($null -eq $CurrentBiosValues)
    {
        return @{
            Verdict          = 'EarlyExitFail'
            EarlyExitStdout  = 'DEGRADED: settings bios-query-failed (HP BIOS setting namespace unreachable)'
            EarlyExitCode    = 1
...
function Invoke-HPSetBiosSetting
{
    #Call HP SetBIOSSetting for one BIOS setting
    ...
    if ($WhatIfPreference)
    {
        ...
        Write-LogEntry -Value "WhatIf: would call HP SetBIOSSetting name=$SettingName value=$SettingValue ($AuthLabel)" -Severity 1
    ...
    $CimArgs = @{
        Name     = $SettingName
        Value    = $SettingValue
        Password = if ([string]::IsNullOrEmpty($Password)) { '<utf-16/>' } else { '<utf-16/>' + $Password }
    }
    try
    {
...
function Get-HPSetFailReason
{
    #Translate an HP write result into a FailReason persisted on the per-setting marker.
...
{
    #Apply the drifted settings: decrypt the unlock password (unless NoPassword), SetBIOSSetting each, verify via a single post-apply readback, then write per-setting + top-level markers.
    ...
    try
    {
        ...
        {
            $Password = ''
            Write-LogEntry -Value "NoPassword mode: applying settings unauthenticated (empty SetBIOSSetting password)" -Severity 1
        }
        else
        {
            try
        ...
        #Phase 1: queue each SetBIOSSetting. Capture per-setting result for use after the post-query.
        $WriteResults = @{}
        foreach ($Item in $SettingsToApply)
        {
            ...
            $Op = Invoke-HPSetBiosSetting -SettingName $Name -SettingValue $DesiredValue -Password $Password
            ...
            {
                Write-LogEntry -Value "SetBIOSSetting returned status=0 for $Name - will verify via post-apply readback" -Severity 1
            }
        }
        ...
        {
            $PostBiosValues = Get-HPBiosSettings
        ...
        foreach ($Item in $SettingsToApply)
        {
            ...
            if ($OpResult.Threw)
            {
                Set-SettingMarker -BasePath $MarkerBasePath -Name $Name -DesiredValue $Desired -LastVerifiedValue $PreValue -Status 'Failed' -FailReason (Get-HPSetFailReason -OpResult $OpResult)
            ...
            {
                Write-LogEntry -Value "Post-apply BIOS does not report setting $Name - marking Failed" -Severity 3
...
Write-LogEntry -Value "START - HP BIOS settings remediation (Intune) v$Version" -Severity 1
...
if (-not $SkipManufacturerCheck)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($Manufacturer) -and $Manufacturer -notlike 'HP*' -and $Manufacturer -notlike 'Hewlett*')
    {
        ...
        Write-LogEntry -Value "END - HP BIOS settings remediation (out of scope)" -Severity 1
        Write-Output 'SKIPPED: settings out-of-scope (not HP)'
        exit 0
    }
...
if ($DesiredSettings.Count -eq 0)
{
    ...
    Write-LogEntry -Value 'END - HP BIOS settings remediation (no work)' -Severity 1
    Write-Output 'SKIPPED: settings no-managed-settings ($DesiredSettings empty - edit the script body)'
    exit 0
}

#Sure Admin short-circuit: signed payloads required, settings remediation does not apply. Exit 0 so Intune does not retry.
$SureAdmin = Get-HPSureAdminEnrolled
if ($SureAdmin -eq $true)
{
    Write-LogEntry -Value "HP Sure Admin (Enhanced BIOS Authentication Mode) is enabled - settings remediation does not apply. Skipping." -Severity 2
    Write-LogEntry -Value "END - HP BIOS settings remediation (Sure Admin incompatible)" -Severity 1
    Write-Output 'SKIPPED: settings sure-admin-incompatible (signed payloads required)'
    exit 0
}
...
if ($Plan.Verdict -eq 'EarlyExitFail')
{
    ...
    Write-LogEntry -Value "END - HP BIOS settings remediation (exit $($Plan.EarlyExitCode))" -Severity 1
...
if ($SettingsToApply.Count -eq 0)
{
    ...
    Write-LogEntry -Value "END - HP BIOS settings remediation (exit 0, no drift)" -Severity 1
    Write-Output $Stdout
    exit 0
}
...
Write-LogEntry -Value "Outcome: $Stdout" -Severity 1
Write-LogEntry -Value "END - HP BIOS settings remediation (exit $ExitCode)" -Severity 1
