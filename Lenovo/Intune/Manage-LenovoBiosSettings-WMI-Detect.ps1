<#
    .DESCRIPTION
        ...
        or 1 (non-compliant / degraded). Detection only reads the BIOS (Lenovo settings reads are
...
param(
    ...
    #Opt-in: BIOS has no supervisor password. Settings are read unauthenticated. Must match the paired -Remediate script.
    [Parameter(Mandatory = $false)]
    [switch]$NoPassword,
    ...
    [System.IO.FileInfo]$LogFile = "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs\Manage-LenovoBiosSettings-WMI-Detect.log",
...
function Get-SettingsClassification
{
    ...
    param(
        ...
        [Parameter(Mandatory = $false)][bool]$PasswordPresent = $false, # NoPassword mode: a BIOS password is actually set
        [Parameter(Mandatory = $false)][AllowNull()][object]$CertAuth, # $true/$false/$null (query failed)
    ...
    #NONCOMPLIANT INCOMPATIBLE - cert-auth enabled, settings cannot be remediated via password.
    if ($CertAuth -eq $true)
    {
        return @{
            Status       = 'NONCOMPLIANT'
            Stdout       = 'NONCOMPLIANT: settings cert-auth-enabled (INCOMPATIBLE - signed payloads required)'
            Reason       = 'Lenovo BIOS certificate-based authentication is in use (PasswordState 128) - settings remediation does not apply'
            DriftedNames = @()
        }
    }

    #DEGRADED - NoPassword configured but a BIOS password is set (unauthenticated writes silently no-op).
    if ($NoPassword -and $PasswordPresent)
    {
        return @{
            Status = 'DEGRADED'
            Stdout = 'DEGRADED: settings nopassword-password-set (BIOS password present; unauthenticated writes silently fail)'
            Reason = 'NoPassword mode but a BIOS password is set - unauthenticated settings writes would silently no-op'
            DriftedNames = @()
        }
    }

    #DEGRADED - pw marker missing (settings remediation cannot authorize)
    if (-not $NoPassword -and -not $PasswordMarkerPresent)
    {
        return @{
            ...
            Reason = 'pw marker missing - settings remediation has no way to authorize SaveBiosSettings'
            DriftedNames = @()
        }
    }
    ...
    if ($null -eq $CurrentBiosValues)
    {
        return @{
            Status = 'DEGRADED'
            Stdout = 'DEGRADED: settings bios-query-failed (Lenovo BIOS setting namespace unreachable)'
...
if (-not $SkipManufacturerCheck)
{
    ...
    if (-not [string]::IsNullOrWhiteSpace($Manufacturer) -and $Manufacturer -notlike 'LENOVO*')
    {
        ...
        Write-LogEntry -Value "END - Lenovo BIOS settings detection (out of scope)" -Severity 1
        Write-Output 'COMPLIANT: settings out-of-scope (not Lenovo)'
        exit 0
    }
...
if ($DesiredSettings.Count -eq 0)
{
    ...
    Write-LogEntry -Value 'END - Lenovo BIOS settings detection (no work)' -Severity 1
...
try
{
    ...
    if ($null -eq $CertAuth)
    {
        Write-LogEntry -Value "Cert-auth query failed - proceeding with normal settings classification" -Severity 2
    }
    elseif ($CertAuth)
    {
        Write-LogEntry -Value "Lenovo cert-auth (PasswordState=128) is ENABLED - device is INCOMPATIBLE with settings remediation" -Severity 2
    }
    else
    {
        Write-LogEntry -Value "Lenovo cert-auth is not enabled - normal settings remediation applies" -Severity 1
    }

    $CurrentBiosValues = Get-LenovoBiosSettings
    if ($null -eq $CurrentBiosValues)
    {
        Write-LogEntry -Value "Lenovo BIOS setting query returned null (DEGRADED)" -Severity 2
    }
    else
    {
        Write-LogEntry -Value "Lenovo BIOS setting query returned $($CurrentBiosValues.Count) settings" -Severity 1
    }

    #NoPassword precondition: detect a present BIOS password up front (unauthenticated writes silently succeed).
    ...
    if ($NoPassword)
    {
        ...
        if ($null -eq $PwState)
        {
            Write-LogEntry -Value "NoPassword: PasswordState query failed - cannot confirm absence of a BIOS password" -Severity 2
        }
        else
        {
            $PasswordPresent = ($PwState -ne 0 -and $PwState -ne 128)
            Write-LogEntry -Value "NoPassword: PasswordState=$PwState (PasswordPresent=$PasswordPresent)" -Severity 1
        }
    }
...
Write-LogEntry -Value "END - Lenovo BIOS settings detection" -Severity 1
