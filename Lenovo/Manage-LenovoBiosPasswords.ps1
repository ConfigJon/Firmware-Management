<#
    ...
    .PARAMETER HDDMasterPasswordCmsFile
        Specify the path to a CMS-encrypted file containing the current master hard drive password to clear. The file is decrypted in memory at runtime using the device's document-encryption certificate. Use this instead of HDDMasterPassword. Cannot be combined with HDDMasterPassword.
    ...
    .CHANGELOG
        ...
        2.3.0 (2026-05-25)
            ...
            - Fixed power on password set, change, and clear on systems using the WMI opcode interface, which previously could silently skip the requested change and fail at the
              next reboot with error 0191. Setting an initial power on or system management password is now also supported when a higher-authority password (supervisor, or for power
              on, system management) is present to authorize it. Power on password operations are validated by the firmware only at the next reboot, so an incorrect authorizing
              password is reported as success at run time and rejected later as 0191; system management password operations are validated synchronously.
            - Fixed a bug where a failed hard drive password clear was logged but not reflected in the script exit code or user prompts; the result is now included in the failure
              handling like the other password types.

        2.2.0 (2026-05-23)
            - Added support for the modern Lenovo WMI opcode interface, required for complex/special-character passwords on 2020+ ThinkPad and 2017+ ThinkCentre/ThinkStation systems.
              The script auto-detects the interface and falls back to the legacy method on older systems. Passwords containing commas and other special characters are now handled
              correctly. Parameters and overall behavior are unchanged.

        2.1.0 (2026-05-23)
            - Added return code checking and logging to the hard drive password clear operations, which previously ran without reporting success or failure.
            - Added graceful handling for PasswordState 128 (BIOS certificate-based authentication in use).

        2.0.0 (2026-05-22)
            - Migrated all WMI access from Get-WmiObject to Get-CimInstance, and Lenovo_SetBiosPassword.SetBiosPassword calls to Invoke-CimMethod, so the script runs on both Windows PowerShell 5.1 and PowerShell 7.
            - Added [CmdletBinding(PositionalBinding=$false)] so all arguments must be named.
            - Fixed a bug where the LenovoSetSystemManagement task sequence variable was checked but never read.

        Pre-2.0.0 (legacy, date-keyed)
...
param(
    ...
    [Parameter(Mandatory=$false)][ValidateNotNullOrEmpty()][String[]]$OldSupervisorPasswordCmsFile,
    ...
    [Parameter(Mandatory=$false)][ValidateNotNullOrEmpty()][String[]]$OldSystemManagementPasswordCmsFile,
    [Parameter(Mandatory=$false)][ValidateNotNullOrEmpty()][String]$HDDUserPasswordCmsFile,
    [Parameter(Mandatory=$false)][ValidateNotNullOrEmpty()][String]$HDDMasterPasswordCmsFile,
...
Function Invoke-LenovoSetPassword
{
    #Change or clear a Lenovo BIOS password. Uses the modern WMI opcode interface when available (handles complex/special-character passwords), otherwise the legacy Lenovo_SetBiosPassword method. An empty NewPassword clears the password. Returns the BIOS status string.

    param(
        [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$PasswordName,
        [Parameter(Mandatory=$true)][ValidateNotNullOrEmpty()][String]$CurrentPassword,
        [Parameter(Mandatory=$false)][AllowEmptyString()][String]$NewPassword = ""
    )
    if($UseOpcode)
    {
        #ThinkCentre/ThinkStation require the current password to be supplied via WmiOpcodePasswordAdmin before a password change. ThinkPad systems do not.
        if($IsDesktop)
        {
            [void](Invoke-CimMethod -InputObject $OpcodeInterface -MethodName WmiOpcodeInterface -Arguments @{Parameter = "WmiOpcodePasswordAdmin:$CurrentPassword;"})
        }
        [void](Invoke-CimMethod -InputObject $OpcodeInterface -MethodName WmiOpcodeInterface -Arguments @{Parameter = "WmiOpcodePasswordType:$PasswordName;"})
        [void](Invoke-CimMethod -InputObject $OpcodeInterface -MethodName WmiOpcodeInterface -Arguments @{Parameter = "WmiOpcodePasswordCurrent01:$CurrentPassword;"})
        [void](Invoke-CimMethod -InputObject $OpcodeInterface -MethodName WmiOpcodeInterface -Arguments @{Parameter = "WmiOpcodePasswordNew01:$NewPassword;"})
        #The SetUpdate opcode commits the change and returns the authoritative status. The change takes effect on the next reboot.
        return (Invoke-CimMethod -InputObject $OpcodeInterface -MethodName WmiOpcodeInterface -Arguments @{Parameter = "WmiOpcodePasswordSetUpdate;"}).Return
    }
    else
    {
        return (Invoke-CimMethod -InputObject $PasswordSet -MethodName SetBiosPassword -Arguments @{parameter = "$PasswordName,$CurrentPassword,$NewPassword,ascii,us"}).Return
    }
}

Function Test-LenovoBiosPassword
{
    #Returns "Success" if the supplied password is the current value for the given password type. On opcode-capable systems only the supervisor password (pap) can be checked non-destructively (WmiOpcodePasswordAdmin); an empty string is returned for all other types, because there is no non-destructive opcode check for them and a set-to-self check would queue an invalid pending change that the firmware reports as success but then rejects with a 0191 error at the next reboot. Note that a successful WmiOpcodePasswordAdmin call disables the password-change opcodes for the rest of the session, so this check is only safe for the supervisor already-set short-circuit, where no further change is attempted. On legacy systems a set-to-self check is used for all password types.
    ...
    if($UseOpcode)
    {
        if($PasswordName -eq "pap")
        {
            return (Invoke-CimMethod -InputObject $OpcodeInterface -MethodName WmiOpcodeInterface -Arguments @{Parameter = "WmiOpcodePasswordAdmin:$Password;"}).Return
        }
        else
        {
            return ""
    ...
    {
        return (Invoke-LenovoSetPassword -PasswordName $PasswordName -CurrentPassword $Password -NewPassword $Password)
    }
}
...
Function Set-LenovoBiosPassword
{
    ...
    if((Test-LenovoBiosPassword -PasswordName $PasswordName -Password $Password) -eq "Success")
    {
    ...
    else
    {
        if($OldPassword)
        {
            ...
            {
                if((Invoke-LenovoSetPassword -PasswordName $PasswordName -CurrentPassword $OldPassword[$Counter] -NewPassword $Password) -eq "Success")
                {
...
Function Clear-LenovoBiosPassword
{
    ...
    While($Counter -lt $OldPassword.Count)
    {
        if((Invoke-LenovoSetPassword -PasswordName $PasswordName -CurrentPassword $OldPassword[$Counter]) -eq "Success")
        {
...
Write-LogEntry -Value "START - Lenovo BIOS password management script (version $Version)" -Severity 1
...
if($OldSupervisorPassword -and $OldSupervisorPasswordCmsFile)
{
    Stop-Script -ErrorMessage "Specify either the OldSupervisorPassword or the OldSupervisorPasswordCmsFile parameter, not both"
}
...
if($OldSystemManagementPassword -and $OldSystemManagementPasswordCmsFile)
{
    Stop-Script -ErrorMessage "Specify either the OldSystemManagementPassword or the OldSystemManagementPasswordCmsFile parameter, not both"
}
if($HDDUserPassword -and $HDDUserPasswordCmsFile)
{
    Stop-Script -ErrorMessage "Specify either the HDDUserPassword or the HDDUserPasswordCmsFile parameter, not both"
}
if($HDDMasterPassword -and $HDDMasterPasswordCmsFile)
{
    Stop-Script -ErrorMessage "Specify either the HDDMasterPassword or the HDDMasterPasswordCmsFile parameter, not both"
}
...
if($OldSupervisorPasswordCmsFile)
{
    Write-LogEntry -Value "Decrypting the old supervisor password(s) from the supplied CMS file(s)" -Severity 1
    $OldSupervisorPassword = Get-CmsPassword -CmsFile $OldSupervisorPasswordCmsFile
}
...
if($OldSystemManagementPasswordCmsFile)
{
    Write-LogEntry -Value "Decrypting the old system management password(s) from the supplied CMS file(s)" -Severity 1
    $OldSystemManagementPassword = Get-CmsPassword -CmsFile $OldSystemManagementPasswordCmsFile
}
if($HDDUserPasswordCmsFile)
{
    Write-LogEntry -Value "Decrypting the user hard drive password from the supplied CMS file" -Severity 1
    $HDDUserPassword = Get-CmsPassword -CmsFile $HDDUserPasswordCmsFile
}
if($HDDMasterPasswordCmsFile)
{
    Write-LogEntry -Value "Decrypting the master hard drive password from the supplied CMS file" -Severity 1
    $HDDMasterPassword = Get-CmsPassword -CmsFile $HDDMasterPasswordCmsFile
}
...
#Determine whether the modern WMI opcode interface is available (required for complex passwords on newer ThinkPad/ThinkCentre/ThinkStation systems). Fall back to the legacy method when it is not present.
$UseOpcode = $false
$IsDesktop = $false
...
if(($null -ne $OpcodeInterface) -and ($OpcodeInterface.Active -eq $true))
{
    ...
    Write-LogEntry -Value "The Lenovo WMI opcode interface is available. Password operations will use the opcode method (supports complex passwords)" -Severity 1
}
else
{
    Write-LogEntry -Value "The Lenovo WMI opcode interface is not available. Password operations will use the legacy method" -Severity 1
}
...
switch($PasswordSettings.PasswordState)
{
    ...
    {$_ -eq 128}
    {
        ...
        Write-Output "BIOS certificate-based authentication is in use. No password actions were taken."
...
if(Get-TaskSequenceStatus)
{
    ...
    $LenovoSetSystemManagement = $TSEnv.Value("LenovoSetSystemManagement")
...
if(!$PopSet)
{
    if($PowerOnSet -and !($SvpSet -or $SmpSet))
    {
        $PowerOnPWExists = "Failed"
        Write-LogEntry -Value "No power on password is currently set, and no supervisor or system management password is available to authorize setting one. Unable to set the power on password" -Severity 3
    }
    if($PowerOnClear)
    {
...
if(!$SmpSet)
{
    if($SystemManagementSet -and !($SvpSet))
    {
        $SystemManagementPWExists = "Failed"
        Write-LogEntry -Value "No system management password is currently set, and no supervisor password is available to authorize setting one. Unable to set the system management password" -Severity 3
    }
...
#Set or change the power on password. A supervisor or system management password (when set) authorizes the operation, which also allows setting an initial power on password. Otherwise the old power on password is used.
if(($PowerOnSet) -and ($LenovoSetPowerOn -ne "Success") -and ($PowerOnPWExists -ne "Failed"))
{
    if($SvpSet)
    {
...
if(($PowerOnClear) -and ($PopSet) -and ($LenovoClearPowerOn -ne "Success"))
{
    if($SvpSet)
    {
...
#Set or change the system management password. A supervisor password (when set) authorizes the operation, which also allows setting an initial system management password. Otherwise the old system management password is used.
if(($SystemManagementSet) -and ($LenovoSetSystemManagement -ne "Success") -and ($SystemManagementPWExists -ne "Failed"))
{
    if($SvpSet)
    {
...
if(($SystemManagementClear) -and ($SmpSet) -and ($LenovoClearSystemManagement -ne "Success"))
{
    if($SvpSet)
    {
...
if((($SupervisorPWSet -eq "Failed") -or ($SupervisorPWClear -eq "Failed") -or ($PowerOnPWSet -eq "Failed") -or ($PowerOnPWClear -eq "Failed") -or ($SystemManagementPWSet -eq "Failed") -or ($SystemManagementPWClear -eq "Failed") -or ($HDDUserPWClear -eq "Failed") -or ($HDDMasterPWClear -eq "Failed")) -and (!($SMSTSPasswordRetry)))
{
    if(!($NoUserPrompt))
    {
        Write-LogEntry -Value "Failures detected, display on-screen prompts for any required manual actions" -Severity 2
        ...
        if($HDDUserPWClear -eq "Failed")
        {
            Start-UserPrompt -BodyText "The user hard drive password is set, but cannot be automatically cleared. Please reboot the computer and manually clear the user hard drive password." -TitleText "Lenovo Password Management Script"
        }
        if($HDDMasterPWClear -eq "Failed")
        {
            Start-UserPrompt -BodyText "The master hard drive password is set, but cannot be automatically cleared. Please reboot the computer and manually clear the master hard drive password." -TitleText "Lenovo Password Management Script"
        }
    }
    #Exit the script with an error
    if(!($ContinueOnError))
...
elseif((($SupervisorPWExists -eq "Failed") -or ($SupervisorPWSet -eq "Failed") -or ($SupervisorPWClear -eq "Failed") -or ($PowerOnPWExists -eq "Failed") -or ($PowerOnPWSet -eq "Failed") -or ($PowerOnPWClear -eq "Failed") -or ($SystemManagementPWSet -eq "Failed") -or ($SystemManagementPWClear -eq "Failed") -or ($HDDUserPWClear -eq "Failed") -or ($HDDMasterPWClear -eq "Failed")) -and ($SMSTSPasswordRetry))
{
